...
import com.example.wsd_client.customViews.CustomListView;
import com.example.wsd_client.customViews.CustomListView.DownRefreshListener;
import com.example.wsd_client.customViews.CustomListView.UpRefreshListener;
...
OnClickListener,OnItemClickListener, OnScrollListener,TextWatcher,DownRefreshListener,UpRefreshListener{

	private TextView tvTitle;
	private CircleImageView civ;
	private ImageButton ibBack;
	private ImageButton ibCart;
	private CustomListView lvLiebiao;
	 ...
	 * 标记变量，用于标记加载时是否有更多数据
	 */
	private boolean flag_more_data = false;
	/**
	 * 标记变量，用于控制加载数据完成时候，加载动画是否启动，避免listview刷新时候，出现dialog空指针异常
	 */
	private boolean flag_load_data_callback = true;
	...
	private void init() {
		...
		lvLiebiao=(CustomListView) view.findViewById(R.id.lv_liebiao);
	...
	private void initListeners(){
		...
		//为customlistview添加上拉赫下拉刷新监听
		lvLiebiao.setDownRefreshListener(this);
		lvLiebiao.setUpRefreshListener(this);
	}
	...
	public void showData() {
		...
		if(flag_load_data_callback){
			loadDataCompleted();
		}
		//进行标记变量的改变,只进行初次加载数据的时候进行加载动画的开启
		flag_load_data_callback = false;
	}
	...
	public void upLoadData(CustomListView listview) {
		int acount = lists.size();
		//进行数据再次请求，如果请求前后数据数量一样，说明没有新数据，进行提示
		if(!flag_more_data){
			presenter.loadData();
		}
		//测试数据不多，暂且这样处理
		if(lists.size()==acount){
			Myapplication.toast("亲，没有更多数据");
			flag_more_data = true;
		}
		//加载完成，恢复listview
